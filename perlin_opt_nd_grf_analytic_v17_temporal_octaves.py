...
def seismic_swarm_rff_analytic(x0, n_steps=2000, n_particles=10, dt=0.01, 
    ...
    t = 0.0
    # Con freq=2.0 base, 1 ciclo toma t=pi. 
    # Queremos alcanzar n_cycles exactos a lo largo de n_steps.
    ...
    for step in range(n_steps):
        ...
        # Octavas Temporales (Fractal Time Tremors)
        f = 2.0
        A = noise_amplitude * decay
        
        # 1 onda seno de amplitud A, periodo T (freq f)
        # 1 onda seno de amplitud A/2, periodo T/2 (freq 2f)
        # 1 onda seno de amplitud A/4, periodo T/4 (freq 4f)
        amp = A * np.sin(t * f) + (A / 2.0) * np.sin(t * 2.0 * f) + (A / 4.0) * np.sin(t * 4.0 * f)
