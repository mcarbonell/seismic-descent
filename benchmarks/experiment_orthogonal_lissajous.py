"""
experiment_orthogonal_lissajous.py — Benchmark comparing Orthogonal Lissajous against RFF and Coordinate Lissajous.

Evaluates:
  1. Seismic (RFF Canonical)
  2. Seismic (Lissajous Direct)
  3. Seismic (Lissajous Coupled)
  4. Seismic (Lissajous Orthogonal)
  5. CMA-ES
"""
...
def main():
    parser = argparse.ArgumentParser(description="Orthogonal Lissajous Benchmark")
    ...
    print(f"\n==========================================================================")
    print(f" ORTHOGONAL LISSAJOUS WAVES VS RFF & COORDINATE LISSAJOUS")
    ...
    for fkey in args.functions:
        ...
        for dims in args.dims:
            ...
            algos = {
                ...
                "Seismic (Lissajous Orthogonal)": {
                    "field_builder": lambda s: OrthogonalLissajousWaveField(dim=dims, n_rotations=3, n_harmonics=2, base_lengthscale=0.4),
    ...
    json_path = results_dir / "orthogonal_lissajous.json"
    ...
    for r_idx, fkey in enumerate(args.functions):
        ...
        for c_idx, dims in enumerate(args.dims):
            ...
            for aname, adata in test_data.items():
                ...
                c = adata["color"]
                ls = adata["style"]
                lw = 2.6 if "Orthogonal" in aname else (1.8 if "Canonical" in aname else 1.2)
                ax.plot(eval_grid, med, label=aname, color=c, linestyle=ls, linewidth=lw)
                if "Orthogonal" in aname or "Canonical" in aname:
            ...
            if r_idx == 0 and c_idx == n_dims - 1:
                ax.legend(frameon=True, facecolor="white", edgecolor="#ddd", fontsize=7.5, loc="upper right")

    plt.suptitle("Orthogonal Lissajous Waves vs Coordinate Lissajous & RFF Baselines", fontsize=14, fontweight="bold", y=0.995)
    plt.tight_layout()
    plot_path = results_dir / "orthogonal_lissajous.png"
    plt.savefig(plot_path, dpi=180, bbox_inches="tight")
    plt.close()
    print(f"[+] Comparative plot saved to: {plot_path}")

    # Generate Markdown report
    md_path = results_dir / "orthogonal_lissajous_report.md"
    md = []
    md.append("# Estudio Comparativo: Ondas de Lissajous Ortogonales vs RFF y Lissajous Directo")
    md.append("")
    md.append(f"> **Configuración:** {args.trials} repeticiones independientes por prueba | Presupuesto: {args.budget} evaluaciones.")
    md.append("")
    md.append("![Orthogonal Lissajous](orthogonal_lissajous.png)")
    md.append("")
    ...
    for tkey, tdata in results["tests"].items():
        ...
        for aname, adata in tdata["algorithms"].items():
            is_ortho = "Orthogonal" in aname
            bold = "**" if is_ortho else ""
