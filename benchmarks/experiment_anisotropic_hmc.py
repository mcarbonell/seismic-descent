"""
experiment_anisotropic_hmc.py — Benchmark for Paso 3: Anisotropic Metric Adaptation and Hamiltonian Dynamics (HMC).

Systematic Ablation Study:
  1. Seismic Canonical (Isotropic, No HMC, No Gravity)
  2. Seismic + Gravity (Phase-Modulated, gamma=0.4)
  3. Seismic + Momentum HMC (mu=0.7, isotropic, gamma=0.4)
  4. Seismic + Anisotropic Metric (alpha=0.5, no momentum, gamma=0.4)
  5. Seismic Anisotropic HMC + Gravity (Full Paso 3: mu=0.7, alpha=0.5, gamma=0.4)
  6. CMA-ES Baseline
"""
...
from seismic_descent.anisotropic_hmc import SeismicAnisotropicHMC
...
def main():
    parser = argparse.ArgumentParser(description="Anisotropic HMC Benchmark")
    ...
    print(f"\n==========================================================================")
    print(f" ANISOTROPIC METRIC ADAPTATION & HAMILTONIAN DYNAMICS (HMC) BENCHMARK")
    ...
    for fkey in args.functions:
        ...
        for dims in args.dims:
            ...
            algos = {
                "Seismic Canonical": {
                    "builder": lambda s: SeismicAnisotropicHMC(
                        bounds=bounds, n_particles=n_particles, n_steps=n_steps,
                        momentum_base=0.0, anisotropic_power=0.0, gravity_strength=0.0, seed=s
                    ),
                    "color": "#94a3b8",  # Gray
                    "style": ":",
                },
                "Seismic + Gravity": {
                    "builder": lambda s: SeismicAnisotropicHMC(
                        bounds=bounds, n_particles=n_particles, n_steps=n_steps,
                        momentum_base=0.0, anisotropic_power=0.0, gravity_strength=0.4, seed=s
                    ),
                    "color": "#0ea5e9",  # Sky Blue
                    "style": "--",
                },
                "Seismic + Momentum HMC": {
                    "builder": lambda s: SeismicAnisotropicHMC(
                        bounds=bounds, n_particles=n_particles, n_steps=n_steps,
                        momentum_base=0.7, anisotropic_power=0.0, gravity_strength=0.4, seed=s
                    ),
                    "color": "#f59e0b",  # Amber
                    "style": "-.",
                },
                "Seismic + Aniso Metric": {
                    "builder": lambda s: SeismicAnisotropicHMC(
                        bounds=bounds, n_particles=n_particles, n_steps=n_steps,
                        momentum_base=0.0, anisotropic_power=0.5, gravity_strength=0.4, seed=s
                    ),
                    "color": "#10b981",  # Emerald
                    "style": "--",
                },
                "Seismic Aniso-HMC (Full)": {
                    "builder": lambda s: SeismicAnisotropicHMC(
                        bounds=bounds, n_particles=n_particles, n_steps=n_steps,
                        momentum_base=0.7, anisotropic_power=0.5, gravity_strength=0.4, seed=s
                    ),
                    "color": "#8b5cf6",  # Violet
                    "style": "-",
                },
    ...
    json_path = results_dir / "anisotropic_hmc.json"
    ...
    for r_idx, fkey in enumerate(args.functions):
        ...
        for c_idx, dims in enumerate(args.dims):
            ...
            for aname, adata in test_data.items():
                ...
                c = adata["color"]
                ls = adata["style"]
                lw = 2.6 if "Full" in aname else (2.0 if "Gravity" in aname else 1.4)
                ax.plot(eval_grid, med, label=aname, color=c, linestyle=ls, linewidth=lw)
                if "Full" in aname or "CMA-ES" in aname:
    ...
    plt.suptitle("Anisotropic Metric Adaptation & Hamiltonian Dynamics (HMC) Ablation", fontsize=14, fontweight="bold", y=0.995)
    plt.tight_layout()
    plot_path = results_dir / "anisotropic_hmc.png"
    plt.savefig(plot_path, dpi=180, bbox_inches="tight")
    plt.close()
    print(f"[+] Comparative plot saved to: {plot_path}")

    # Generate Markdown report
    md_path = results_dir / "anisotropic_hmc_report.md"
    md = []
    md.append("# Estudio Comparativo: Adaptación Anisótropa y Dinámica Hamiltoniana (HMC)")
    md.append("")
    md.append(f"> **Configuración:** {args.trials} repeticiones independientes por prueba | Presupuesto: {args.budget} evaluaciones.")
    md.append("")
    md.append("![Anisotropic HMC](anisotropic_hmc.png)")
    md.append("")
    ...
    for tkey, tdata in results["tests"].items():
        ...
        for aname, adata in tdata["algorithms"].items():
            is_full = "Full" in aname
            bold = "**" if is_full else ""
