"""
benchmark_suite_v20.py — Suite de benchmark para Seismic Descent v20.

Demuestra el uso de HIPERPARÁMETROS UNIVERSALES gracias a la normalización
de dominio (X) y de gradiente (Y).
"""
...
def run_single_benchmark(func_config, dims, n_trials, eval_budget_base):
    ...
    # =================================================================
    # HIPERPARÁMETROS UNIVERSALES (LA PRUEBA DE FUEGO)
    # Cero lógica condicional por función. El mismo dt y ruido para todas.
    # =================================================================
    UNIVERSAL_DT = 0.2         # 20% del dominio normalizado
    UNIVERSAL_NOISE_AMP = 0.5  # Ruido más bajo
    DT_CYCLES_MULT = 5.0       # Ciclo de dt 5 veces más rápido que el sismo
    ...
    for trial in range(n_trials):
        ...
        _, sval, _ = seismic_swarm(
            ...
            dt_cycles_multiplier=DT_CYCLES_MULT
        )
...
def generate_markdown(all_results, output_file):
    md = "# Benchmark Suite v20 Results (Universal Hyperparameters)\n\n"
    for res in all_results:
        md += f"## {res['function'].capitalize()} {res['dims']}D — {res['n_trials']} trials | budget={res['eval_budget']} evals (threshold: {res['threshold']})\n\n"
        md += "| Algoritmo | Éxitos | Media | Mediana | Std | Tiempo |\n"
        md += "|---|---|---|---|---|---|\n"
        
        for algo in ['seismic', 'sa', 'cmaes']:
            data = res['results'][algo]
            name = "Seismic v20" if algo == 'seismic' else ("SA" if algo == 'sa' else "CMA-ES")
...
if __name__ == '__main__':
    ...
    with open('results/benchmark_suite_v20.json', 'w', encoding='utf-8') as f:
        f.write(json.dumps(all_results, indent=2))
        
    generate_markdown(all_results, 'results/benchmark_suite_v20.md')
