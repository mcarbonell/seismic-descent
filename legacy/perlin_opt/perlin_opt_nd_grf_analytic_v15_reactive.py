...
def seismic_swarm_rff_analytic(x0, n_steps=2000, n_particles=10, dt=0.01, 
    ...
    # Parámetros del sismo reactivo
    state = "SETTLING" 
    E_steps = max(20, n_steps // 10)  # Un terremoto toma el 10% del budget
    dt_noise = np.pi / E_steps        # Avance de fase para completar sin(2*t)
    earthquake_step = 0
    earthquake_phase = 0.0
    
    # Ventana de estancamiento
    history_window = max(10, n_steps // 20)
    best_vals_window = []
    ...
    for step in range(n_steps):
        ...
        best_vals_window.append(best_val)
        if len(best_vals_window) > history_window:
            best_vals_window.pop(0)

        # Máquina de estados
        if state == "SETTLING":
            amp = 0.0
            if len(best_vals_window) == history_window:
                improvement = best_vals_window[0] - best_vals_window[-1]
                if improvement < 1e-3: # Estancamiento detectado
                    state = "EARTHQUAKE"
                    earthquake_step = 0
                    earthquake_phase = 0.0
        ...
        if state == "EARTHQUAKE":
            freq = 2.0
            amp = noise_amplitude * np.sin(earthquake_phase * freq)
            earthquake_phase += dt_noise
            earthquake_step += 1
            
            if earthquake_step >= E_steps:
                state = "SETTLING"
                best_vals_window = [] # Reset para dejar enfriar

        # Calcular gradientes
        f_grad = rastrigin_grad_nd_vec(X)      # (N, D)
        
        if amp != 0.0:
            # global_t avanza siempre un poco para que el RFF mute constantemente
            noise_grad = rff_noise_and_grad_nd_vec(X, step * 0.01, amp) 
...
def run_benchmark(dims, n_trials=30, threshold=None, n_particles=None):
