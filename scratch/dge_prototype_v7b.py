...
# =============================================================================
# DGE v7b — con lr escalado por dimension
# =============================================================================

class DGEOptimizerV7b:
    """
    DGE v7b: Adam + lr escalado + Gradient Clipping Adam + lr escalado por 1/sqrt(D) para estabilidad en alta dimension.
    ...
    def step(self, f, x):
        ...
        if update_norm > self.max_norm:
            adam_update = adam_update * (self.max_norm / update_norm)
...
def benchmark(f, dim, x0, optimum, total_evals,
              dge_lr=0.5, dge_delta=1e-2, dge_greedy=0.3, dge_max_norm=1.0,
    ...
    # --- DGE -----------------------------------------------------------------
    dge_opt = DGEOptimizerV7b(
        dim=dim, lr=dge_lr, delta=dge_delta, greedy_w=dge_greedy, max_norm=dge_max_norm,
        lr_decay=0.01, delta_decay=0.05, total_steps=dge_steps, seed=seed,
    )
    x_dge = x0.copy()
    dge_result = BenchResult("DGE v7b")
    evals_dge = 0
